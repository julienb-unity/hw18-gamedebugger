...
{
    public class HorizontalDragger : MouseManipulator
    {
        Vector2 m_Start;
        bool m_Active;

        Vector2 panSpeed { get; set; }

        bool clampToParentEdges { get; set; }

        public HorizontalDragger()
        {
        ...
        void OnMouseMove(MouseMoveEvent e)
        {
            ...
            if (m_Active)
            {
                ...
                Rect rect = CalculatePosition(target.layout.x + diff.x, target.layout.y, target.layout.width, target.layout.height);
